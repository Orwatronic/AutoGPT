...
from backend.blocks.http import HttpCredentials, HttpMethod, SendWebRequestBlock
...
class TestHttpBlockWithHostScopedCredentials:
    ...
    async def test_http_block_with_exact_host_match(
        ...
        # Prepare input data
        input_data = SendWebRequestBlock.Input(
    ...
    @pytest.mark.asyncio
    @patch("backend.blocks.http.Requests")
    async def test_http_block_without_credentials(
        self,
        mock_requests_class,
        http_block,
        mock_response,
    ):
        """Test HTTP block when no credentials are provided."""
        # Setup mocks
        mock_requests = AsyncMock()
        mock_requests.request.return_value = mock_response
        mock_requests_class.return_value = mock_requests
        ...
        # Execute without credentials (execution manager found no matching ones)
        result = []
        async for output_name, output_data in http_block.run(
            input_data,
            credentials=None,
        ...
        # Verify only user headers are included
        mock_requests.request.assert_called_once()
        call_args = mock_requests.request.call_args
        expected_headers = {"User-Agent": "test-agent"}
        assert call_args.kwargs["headers"] == expected_headers
